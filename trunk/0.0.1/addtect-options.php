...
if ( ! class_exists( 'ganohrs_add_tect_Options' ) ) :
	 ...
	 * @return	   void
	 */
	class ganohrs_addtect_options {

		/**
		 * 設定ページ用の識別ID
		 */
		const PAGE_ID = 'ganohrs-add-tect-options';
		...
		public function use_cookie_modal_callback() {
			$use_cookie_modal = is_array( $this->options ) ? @$this->options['use_cookie_modal'] : '';
		...
		public function use_addetect_modal_callback() {
			$use_addetect_modal = is_array( $this->options ) ? @$this->options['use_addetect_modal'] : '';
